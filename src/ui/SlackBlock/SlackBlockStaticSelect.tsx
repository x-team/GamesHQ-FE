...
const SlackBlockStaticSelect = ({ staticSelectElement, onClose }: IProps) => {
  const onSubmit = async (values: ISlackBlockOption) => {
    await handleGameResponse({
      adminGameRequest: () =>
        postArenaAction(
          staticSelectElement.action_id,
          values.selectedOptionId ? [values.selectedOptionId] : []
        ),
  ...
  const { getFieldProps, handleSubmit, setValues } = useFormik({
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          {staticSelectElement.options.map(option => {
            const label = separateEmojisFromText(option.text)
              .filter(i => !i.emoji)
              .map(i => i.text)
              .join(' ')
