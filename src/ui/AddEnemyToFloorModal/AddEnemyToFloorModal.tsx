              ...
              {allEnemies.map(enemy => {
                return (
                  <span
                    ...
                    key={enemy.id}
                  >
                    {emojiToImageTag(enemy.emoji, allEmoji, 'h-12 w-12')}
                  </span>
                )
              })}
            </div>
          ...
          <div className="w-full bg-green-500 relative rounded-md">
            ...
            {floorEnemies && floorEnemies.length > 0 && (
              <span
                ...
                className="absolute cursor-pointer text-white bottom-2 left-1/2 hover:text-xteamaccent"
                onClick={() => setFloorEnemies([])}
              >
                <AiOutlineDelete />
              </span>
            )}
