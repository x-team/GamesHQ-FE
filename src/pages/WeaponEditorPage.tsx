...
const WeaponEditorPage = function WeaponEditorPage({ editMode }: IProps) {
  ...
  const [remoteWeaponItem, setRemoteWeaponItem] = useState<IWeapon | undefined>(
    undefined
  );
  ...
  const validationSchema = Yup.object({
    ...
    emoji: Yup.string()
      ...
      .label("Emoji"),
    gameTypeIds: Yup.array().of(Yup.number()).min(1).label("Game Availability"),
  ...
  useEffect(() => {
    async function fetchWeapon() {
      ...
      setValues({
        ...
        emoji: weapon.emoji,
        gameTypeIds: weapon._gameItemAvailability.map((gameAvailability) =>
          String(gameAvailability._gameType.id)
        ),
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex mt-4">
          <div className="mr-12">
            <Dropdown fieldProps={getFieldProps("rarity")} label="Rarity">
              <option value="" label="Select a rarity..." disabled />
              {Object.values(RARITY).map((rarity) => (
                <option key={rarity} value={rarity} label={rarity} />
              ))}
            </Dropdown>
          </div>
        </div>
